#!/usr/bin/env python3
"""Check for new GCM releases and signal when a new tag should be created."""
...
def validate_assets(release: dict[str, Any], version: str) -> bool:
    ...
    for platform_key, ext in EXPECTED_ASSETS:
        expected = f"{platform_key}-{version}{ext}"
...
def main() -> None:
    our_repo = os.environ.get("GITHUB_REPOSITORY", "cataggar/pypi-git-credential-manager")
    log.info("Checking for new %s releases...", UPSTREAM_REPO)

    release = get_latest_release()
    tag_name = release["tag_name"]  # e.g. "v2.7.3"
